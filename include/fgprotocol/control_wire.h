...
// JSBSim's FGUDPInputSocket wire format (src/input_output/FGUDPInputSocket.cpp):
// a comma-separated ASCII line, "timestamp,v1,v2,...,vN\n", where each vN is
// the next value in the <input>'s declared <property> list, in order.
// JSBSim drops the whole datagram, silently, on either a comma-count
// mismatch against the property count it was configured with, or a
// timestamp that doesn't strictly increase over the last one it accepted.
//
...
// Builds one FGUDPInputSocket datagram from `count` values, in order.
//
...
// previous call's, bumped via nextafter() if `t` didn't advance enough on
// its own (e.g. two sends landing in the same clock tick, or `t` going
// backwards) -- JSBSim silently drops any packet whose timestamp doesn't
// strictly increase, so this is the actual protocol requirement, not just
// a nicety.
